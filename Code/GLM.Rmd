...
gage_height_slm <- lm(data = USGS_NOAA_combined, gage_height ~ PRCP)
summary(gage_height_lm)
#for every 1(in) of precipitation, there is -0.112 gage height. 0(in) in precip is 5 ft of gage height
#0.02% of lake level is explained by precip
#significant pvalue 0.013

cor.test(USGS_NOAA_combined$gage_height,USGS_NOAA_combined$PRCP)
#t = -2.4773, df = 22640, p-value = 0.01325
# cor -0.0164618 
...
```{r}
gageheight_by_prcp <- ggplot(USGS_NOAA_combined, aes(x = PRCP, y = gage_height)) +
  geom_point()
print(gageheight_by_prcp)
#no linear relationship

ggplot(USGS_NOAA_combined, aes(x = PRCP, y = log(gage_height))) +
  geom_point() +
...
gage_height_slm2 <- lm(data = USGS_NOAA_combined, log(gage_height) ~ PRCP)
summary(gage_height_slm2)
#one unit increase in prcp, a change in log value of gage height is -0.02
#p-value: 0.005774, 22640 DF, Adjusted R-squared:  0.0002923 
...
```{r}
gage_height_slm3 <- lm(data = Tahoe.naomit, gage_height ~ TMIN)
summary(gage_height_slm3)
#p-value: < 2.2e-16 , Adjusted R-squared:  0.02169, 21438 degrees of freedom

par(mfrow = c(2,2), mar=c(1,1,1,1))
plot(gage_height_slm3)
par(mfrow = c(1,1))

ggplot(Tahoe.naomit, aes(x = TMIN, y = log(gage_height))) +
  geom_point()
```

```{r}
gage_height_slm4 <- lm(data = Tahoe.naomit, log(gage_height) ~ TMAX)
summary(gage_height_slm4)
#p-value: < 2.2e-16 , Adjusted R-squared:  0.000625, 21438 degrees of freedom

par(mfrow = c(2,2), mar=c(1,1,1,1))
plot(gage_height_slm4)
par(mfrow = c(1,1))

ggplot(Tahoe.naomit, aes(x = TMAX, y = log(gage_height))) +
  geom_point() +
  geom_smooth(method = "lm")
```
...
cor.test(USGS_NOAA_combined$gage_height, USGS_NOAA_combined$PRCP, method = "spearman", exact = FALSE)
#rho -0.02774514 , S = 1.9883e+12, p-value = 2.974e-05
```

#Multiple Regression

```{r}
gage_height_mlm <- lm(data = USGS_NOAA_combined, gage_height ~ PRCP + TMIN + SNOW)
summary(gage_height_mlm)
#p-value: < 2.2e-16, TMIN and SNOW are significant predictors of gage height 
#Adjusted R-squared:  0.0238  %3 of our data is explained by predictors 
...
Gage_AIC <- lm(data = Tahoe.naomit, gage_height ~ PRCP + SNOW + SNWD + TMAX + TMIN)
step(Gage_AIC)
#lm(formula = gage_height ~ PRCP + SNWD + TMAX + TMIN, data = Tahoe.naomit)
#(Intercept)         PRCP         SNWD         TMAX         TMIN  
   # 4.98141     -0.30882      0.01371     -0.02294      0.06725  

Gage_model_AIC <- lm(data = Tahoe.naomit, gage_height ~ PRCP + SNWD + TMAX + TMIN)
summary(Gage_model_AIC)
#p-value: < 2.2e-16, Adjusted R-squared:  0.03494, 21435 DF
